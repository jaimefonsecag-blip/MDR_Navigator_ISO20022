...
`npm run check` valida veinte cosas:
...
| `check-api-sample` | Que `apiSampleFromName` devuelva ejemplos contextuales (IBAN, BIC, LEI, identificadores, moneda, país, importes…) según el datatype ISO, el nombre del campo y el tag XML; y que `apiSampleLeaf` los use correctamente sin romper el fallback a CodeSets ni a los tipos genéricos |
